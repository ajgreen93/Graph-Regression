...
attr(make_laplacian_eigenmaps_test,"precompute_data") <- function(Y,X,theta_df){
  ...
  precomputed_data <- precompute_laplacian_eigenmaps_data(Y,X,X,X,theta_df)
...
attr(make_spectral_projection_test,"precompute_data") <- function(Y,X,theta_df){
  precompute_least_squares_data(Y,X,X,X,theta_df)
}
...
make_least_squares_test <- function(theta){
  least_squares_test <- function(Y,X){
    # 1. Compute fitted values.
    estimator <- make_least_squares(theta)
    environment(estimator)$precomputed_data <- precomputed_data
    ...
    # 3. Approximate distribution under null via permutation.
    norm_null <- numeric(B)
    for(ii in 1:B){
      estimate_perm <- estimator(sample(Y),X)
      fits_perm <- estimate_perm(X)
      norm_null[ii] <- mean(fits_perm^2)
    }
    
    # 4. Accept or reject.
    mean(norm > norm_null) > 1 - alpha  
  }
}
attr(make_least_squares_test,"precompute_data") <- function(Y,X,theta_df){
    precompute_least_squares_data(Y,X,X,X,theta_df)
}
attr(make_least_squares_test, "compute_threshold") <- function(X,theta_df,precomputed_data){
  norm_null <- matrix(nrow = nrow(theta_df),ncol = B)
  for(bb in 1:B){
    Y_null <- rnorm(nrow(X),0,1)
    for(kk in 1:nrow(theta_df)){
      theta <- slice(theta_df,kk)
      
      # 1. Compute fitted values.
      estimator <- make_least_squares(theta)
      environment(estimator)$precomputed_data <- precomputed_data$train
      estimate <- estimator(Y_null,X)
      fits <- estimate(X)
      
      # 2. Compute L2 norm.
      norm_null[kk,bb] <- mean(fits^2)
    }
  }
  threshold <- apply(norm_null,1,FUN = function(norms){quantile(norms,1 - alpha)})
  return(threshold)
