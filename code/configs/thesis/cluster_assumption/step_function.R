...
### General configs.
d <- 1
ns <- round(seq(400,2000,length.out = 1))
iters <- 1
...
initialize_thetas <- list(
  laplacian_eigenmaps = function(sample_X,n){
    initialize_laplacian_eigenmaps_thetas(sample_X,n,max_K = max_K_graph)
    },
  laplacian_smoothing = function(sample_X,n){
    initialize_laplacian_smoothing_thetas(sample_X,n,max_K = max_K_graph)
    },
