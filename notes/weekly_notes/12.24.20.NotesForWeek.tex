...
\begin{equation*}
\int_{0}^{\varepsilon} \bigl|L_{P,\varepsilon}u(x)\bigr|^2 \,dx \leq C \varepsilon^{2k},
\end{equation*}
with the bound on $\int_{1 - \varepsilon}^{1} \bigl|L_{P,\varepsilon}u\bigr|^2$ following in a completely analogous fashion.

Taking a Taylor expansion of $u$ around $0$ gives
\begin{align*}
u(x) - u(0) & = \sum_{j = 1}^{k - 1} x^j u^{(j)}(0) + \frac{1}{(k - 1)!} \int_{0}^{x}u^{(k)}(s)(x - s)^{k - 1}\,ds \\
& = \frac{1}{(k - 1)!} \int_{0}^{x}u^{(k)}(s)(x - s)^{k - 1}\,ds,
\end{align*}
with the second inequality following since $u \in C_c^{\infty}(\Omega)$ implies $u^{(j)}(0) = 0$ for $j = 1,\ldots,k - 1$. From this identity along with Jensen's inequality, we have
\begin{align*}
\int_{0}^{\varepsilon} \int_{\Omega} \bigl(u(x)\bigr)^2 \eta\biggl(\frac{z - x}{\varepsilon}\biggr) \,dz \,dx & = \int_{0}^{\varepsilon} d_{\nu}(x) \bigl(u(x)\bigr)^2 \,dx \\
& \leq \varepsilon \int_{0}^{\varepsilon} \bigl(u(x)\bigr)^2 \,dx \\
& \leq \varepsilon^{2k} \int_{0}^{\varepsilon} \int_{0}^{x} \bigl(u^{(k)}(s)\bigr)^2 \,ds \,dx. 
\end{align*}
From here, switching the order of integration and substituting $v = s/\varepsilon$ gives the desired result:
\begin{align*}
I_1 & \leq \frac{\varepsilon^{2k - 1}}{\alpha} \int_{0}^{\varepsilon} \int_{s}^{\varepsilon} \bigl(u^{(k)}(s)\bigr)^2 \,dx \,ds \\
& \leq \frac{\varepsilon^{2k + 1}}{\alpha} \int_{0}^{1} \bigl(u^{(k)}(v)\bigr)^2 \,dv \\
& = \frac{\varepsilon^{2k + 1}}{\alpha} |u|_{H^k(\Omega)}^2
\end{align*}
