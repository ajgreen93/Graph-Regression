...
\end{equation*}
where $d_Q(x) = \int_{\Omega} \eta\bigl(\|z - x\|/\varepsilon\bigr) \,dQ(z)$. We denote $T_{\varepsilon,P_n}$ by $T_{\varepsilon,n}$, and $d_{P_n}$ by $d_n$. For convenience, we will assume that $\eta$ is normalized, $\int_{0}^{\infty} \eta(\|z\|) \,dz = 1$, and we will denote $K(\cdot) = (\eta(\cdot))^2$.

The estimate we will consider is a kernel smoother passed over $\wh{\theta}$,
\begin{equation}
...
\begin{lemma}
	Suppose $f^{\ast} \in H_0^{s}(\Omega)$, $p \in C^{\beta}(\Omega)$ for $\beta = s - 1$, and that $\eta$ is an order-$s$ kernel. Then
	\begin{equation*}
	\|T_{P,\varepsilon}f^{\ast} - f^{\ast}\|_P^2 \leq C \varepsilon^{2s}.
	\end{equation*}
\end{lemma}

Thus balancing bias and variance, using a smaller bandwidth ($\varepsilon \asymp n^{-1/(2\beta + d)}$) than is typical for kernel smoothing with noisy data, leads to an asymptotically negligible error ($n^{-2s/(2\beta + d)}$) compared to the minimax rate for estimating Sobolev functions. 
...
\begin{proof}[Proof of Lemma~\ref{lem:kernel_smoothing_laplacian_eigenmaps}]
	...
	\|T_{\varepsilon,n}f^{\ast}_{1:n} - f^{\ast}\|_P^2 \leq \frac{1}{\delta}\frac{\varepsilon^2}{n\varepsilon^d} |f|_{H^1(\mc{X})}^2 + \frac{1}{\delta}\|T_{\varepsilon,P}f^{\ast} - f^{\ast}\|_P^2,
	\end{equation}
	...
	\begin{equation*}
	\Bigl(\bigl(T_{n,\varepsilon}u\bigr)(x)\Bigr)^2 \leq \frac{1}{d_n(x) n} \sum_{i = 1}^{n} u_i^2 \eta\biggl(\frac{\|X_i - x\|}{\varepsilon}\biggr).
	\end{equation*}
	...
	\begin{align*}
	T_{n,\varepsilon}(\wh{\theta} - \varepsilon)(x) & = \frac{1}{d_n(x)n} \sum_{i = 1}^{n} \bigl(\wh{\theta}_i - f^{\ast}(X_i)\bigr) \eta\biggl(\frac{\|X_i - x\|}{\varepsilon}\biggr) \\
	& = \frac{d_n(x;K^{1/2})}{d_n(x)}\frac{1}{d_n(x;K^{1/2})n} \sum_{i = 1}^{n} \bigl(\wh{\theta}_i - f^{\ast}(X_i)\bigr) \mathrm{sgn}\biggl(\eta\biggl(\frac{\|X_i - x\|}{\varepsilon}\biggr)\biggr) K^{1/2}\biggl(\frac{\|X_i - x\|}{\varepsilon}\biggr).
	\end{align*}
	Taking the square and applying Jensen's inequality, we deduce that
	\begin{align*}
	\bigl(T_{n,\varepsilon}(\wh{\theta} - \varepsilon)(x)\bigr)^2 & \leq \biggl(\frac{d_n(x;K^{1/2})}{d_n(x)}\biggr)^2 \cdot \frac{1}{d_n(x;K^{1/2})n}\sum_{i = 1}^{n} \bigl(\wh{\theta}_i - f^{\ast}(X_i)\bigr)^2 K^{1/2}\biggl(\frac{\|X_i - x\|}{\varepsilon}\biggr) \\
	& \leq \frac{(1 + o_P(1))\|\eta\|_{\infty}}{\alpha} \cdot \frac{1}{d_n(x;K^{1/2})n}\sum_{i = 1}^{n} \bigl(\wh{\theta}_i - f^{\ast}(X_i)\bigr)^2 K^{1/2}\biggl(\frac{\|X_i - x\|}{\varepsilon}\biggr),
	\end{align*}
	with the latter inequality following by Lemma~\ref{lem:measure_of_ball_estimate}. From here the proof proceeds along similar lines to the case where $\eta$ is non-negative.
	...
	\end{align*}
	where $E_{\varepsilon}(f^{\ast}) := \int_{\Omega} \int_{\Omega} (f^{\ast}(x') - f^{\ast}(x))^2 K(\|x' - x\|/\varepsilon) p(x') p(x) ,\dx' \,dx$, and we have shown in our AISTATS paper that $E_{\varepsilon}(f^{\ast}) \leq C \varepsilon^{d + 2} \|f\|_{H^1(\Omega)}^2$, justifying the second inequality.
\end{proof}
