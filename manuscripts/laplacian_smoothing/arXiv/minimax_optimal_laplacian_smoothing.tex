...
\newcommand{\agcomment}[1]{{\bf{{\green{{AJG --- #1}}}}}}
...
\begin{enumerate}[label=(R\arabic*)]
	...
	\end{equation*}
\end{enumerate}
We will also require that the radius of the graph Sobolev class \sbcomment{What does the class have to do with graphs?} not be too large. Precisely, we will require $M \leq M_{\max}(d)$, where we define
\begin{equation*}
M_{\max}(d) :=
\begin{cases*}
n^{1/8} & \textrm{$d = 1$} \\
...
\end{equation*}
and the second equality follows using integration by parts.\footnote{Assuming $f$ satisfies e.g., Dirichlet boundary conditions.} To be clear, this argument does not formally imply that $\wh{f}$ and $\wt{f}$ are close \sbcomment{Should remind people what these are, or refer to eqns where they are defined.} (for one, note that \eqref{eqn:seminorm_consistency} holds for $f \in C^2(\Xset)$, whereas the optimization in \eqref{eqn:thin_plate_spline} considers a much broader set of continuous functions with weak derivatives in $\Leb^2(\Xset)$). But it does seem to suggest that the two estimators should behave somewhat similarly. 
