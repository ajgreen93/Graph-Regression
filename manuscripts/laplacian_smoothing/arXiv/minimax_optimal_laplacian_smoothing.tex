...
We show in Appendix~\ref{sec:fixed_graph_error_bounds} that $\wh{f}$ is a level-$\alpha$ test. In the next theorem, we upper bound the worst-case risk $R_n(\wh{\varphi},H^1(\mc{X},M),\epsilon)$ of $\wh{\varphi}$, whenever $\epsilon$ is at least (a constant times) the critical radius given in~\eqref{eqn:sobolev_space_testing_critical_radius}. For this to hold, we will require a tighter range of scalings for the graph radius $r$.
\begin{enumerate}[label=(R\arabic*)]
	\setcounter{enumi}{1}
	\item 
