...
We now establish the upper bound~\eqref{eqn:linear_smoother_fixed_graph_testing_typeII} on Type II error. From assumption~\eqref{eqn:linear_smoother_fixed_graph_testing_critical_radius}, we see that $f_0^{\top} S^2 f_0^{\top} - \wc{t}_b \leq 0$. As a result,
\begin{align*}
...
\end{aligned}
\end{equation*}
---where in the above the maximum is over all indices $k$ such that $\lambda_k > 0$, and the last inequality follows from the basic algebraic identity $1 - 1/(1 + x)^2 \leq 2 x$ for any $x > 0$ \sbcomment{You've used this before, so maybe move this comment earlier?}---we deduce that
\begin{equation*}
...
\agcomment{Siva, I erred on the side of giving too much detail here, figuring it was easier for you/me to pare back if needed.}
\sbcomment{It is extremely insightful actually. I would consider including some of the second paragraph in the main paper too.}
