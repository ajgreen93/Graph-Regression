...
We show in the supplement that \smash{$\wh{f}$} is a level-$\alpha$ test. In the next theorem, we upper bound the worst-case risk \smash{$R_n(\wh{\varphi},H^1(\mc{X},M),\epsilon)$} of \smash{$\wh{\varphi}$}, whenever $\epsilon$ is at least (a constant times) the critical radius given in~\eqref{eqn:sobolev_space_testing_critical_radius}. For this to hold, we will require a tighter range of scalings for the graph radius $r$.
\begin{enumerate}[label=(R\arabic*)]
	\setcounter{enumi}{1}
	\item 
