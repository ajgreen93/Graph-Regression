...
\begin{table}
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Summary of testing rates over first-order Sobolev balls; black, red, and bold fonts are used as in Table~\ref{tbl:estimation_rates}. The rates for thin-plate splines with $d \geq 2$ assume the test statistic \smash{$\wt{T}$} is computed using an \smash{$\wt{f}$} that interpolates the responses, \smash{$\wt{f}(X_i) = Y_i$} for $i = 1,\ldots,n$.  Rates for $d \geq 4$ assume that $f_0 \in \Leb^4(\Xset,M)$. Lastly, when $\Xset$ is an $m$-dimensional manifold embedded in $\Rd$, all rates hold with $d$ replaced by $m$.}
...
\end{equation*} 
where in the above the infimum is over all level-$\alpha$ tests $\phi$, and $\Ebb_{f_0}[\cdot]$ is the expectation operator under the regression function $f_0$.\footnote{Clearly, the minimax critical radius $\epsilon(\mc{F})$ depends on $\alpha$ and $b$. However, we adopt the typical convention of treating $\alpha \in (0,1)$ and $b \geq 1$ as fixed positive constants; hence they will not affect the testing error rates, and we suppress them notationally.} See \citet{ingster82,ingster87,ingster2012} for a more extended treatment of the minimax paradigm in nonparametric testing. 

Testing $f_0 = 0$ is an easier problem than estimating $f_0$, and hence the minimax testing rate over $H^1(\Xset,M)$ is smaller than the minimax estimation rate. For $1 \leq d < 4$, the squared critical radius is (see \citet{ingster2009}):
\begin{equation}
...
We show in the supplement that \smash{$\wh{f}$} is a level-$\alpha$ test. In the next theorem, we upper bound the worst-case risk \smash{$R_n(\wh{\varphi},H^1(\mc{X},M),\epsilon)$} of \smash{$\wh{\varphi}$}, whenever $\epsilon^2$ is at least (a constant times) the squared critical radius given in~\eqref{eqn:sobolev_space_testing_critical_radius}. For this to hold, we will require a tighter range of scalings for the graph radius $r$.
\begin{enumerate}[label=(R\arabic*)]
	\setcounter{enumi}{1}
	\item 
...
\begin{theorem}
	...
	\end{equation} 
	then the worst-case risk satisfies the upper bound: \smash{$R_n(\wh{\varphi},H^1(\mc{X},M), \epsilon) \leq C/b + C_1n\exp(-c_1nr^d)$}.
\end{theorem}
...
\end{equation*}
and the second equality follows using integration by parts.\footnote{Assuming $f$ satisfies e.g., Dirichlet boundary conditions.} To be clear, this argument does not formally imply that the Laplacian smoothing estimator \smash{$\wh{f}$} and the thin-plate spline estimator \smash{$\wt{f}$} are close (for one, note that \eqref{eqn:seminorm_consistency} holds for $f \in C^2(\Xset)$, whereas the optimization in \eqref{eqn:thin_plate_spline} considers a much broader set of continuous functions with weak derivatives in $\Leb^2(\Xset)$). But it does seem to suggest that the two estimators should behave somewhat similarly. 
