...
\begin{theorem}
	...
	Suppose $f_0 \in H^s(\mc{X},M)$, for $s \leq 2$. Under appropriate conditions, with probability at least $1 - \delta - \textcolor{red}{(?)}$,
	\begin{equation}
...
\begin{itemize}
	\item \textbf{Proof strategy.} The proofs of Theorems~\ref{thm:laplacian_eigenmaps_estimation_manifold} and~\ref{thm:laplacian_eigenmaps_testing_manifold} proceed in a similar manner to the full-dimensional setting.
	\item \textbf{Restriction on number of derivatives.} Unlike in the full-dimensional case, Theorem~\ref{thm:laplacian_eigenmaps_estimation_manifold} holds only when $s \leq 2$. Roughly speaking, we need this restriction in order to show that $L_{P,\varepsilon}^sf(x)$ is a good approximation to $\Delta_P^sf(x)$. Otherwise, the error incurred by using Euclidean distance rather than geodesic distance in~\eqref{eqn:nonlocal_laplacian} will dominate.
\end{itemize}
