...
\end{theorem}
From~\eqref{eqn:laplacian_eigenmaps_estimation_fo} it follows immediately that with constant probability $\|\wh{f} - f_0\|_n^2 \lesssim M^2(M^2n)^{-2/(2 + d)}$ whenever $n^{-1/2} \lesssim M \lesssim n^{1/d}$, matching the standard minimax estimation rate over Sobolev classes.
...
% (AG 8/19/21): Need to include some discussion of how the analysis contrasts with that of the classical spectral projection estimator and test.
