...
\end{enumerate}
Under assumptions~\ref{asmp:domain}-\ref{asmp:sobolev_radius}, the minimax estimation rate over $\mc{H} = H^s(\mc{X};M)$ or $\mc{H} = H_0^s(\mc{X};M)$) is (see e.g. \citet{tsybakov08})
\begin{equation}
...
\textcolor{red}{(TODO)}
\begin{itemize}
	\item Is there a better citation than \citet{tsybakov08} for the minimax rate of estimation~\eqref{eqn:minimax_estimation_rate}? Tsybakov considers the univariate case with fixed, equispaced design.
...
\textbf{(1.4) Rates for testing with higher-order smoothness assumptions.} For testing, we can get the optimal rate $n^{-4s/(4s + d)}$ when $d \leq 4$. When $4 < d \leq 4s$, we get the rate~\textcolor{red}{(...)} Recall that the rate $n^{-4s/(4s + d)}$ is optimal whenever $d \leq 4s$. Thus when $4 < d < 4s$, our upper bounds do not imply that $\varphi$ is an optimal test. We do not know whether this is due to looseness in our proof techniques, or a problem with the test itself, and leave this for future work.
...
Some remarks:
\begin{itemize}
	\item \textbf{Tuning parameters by sample-splitting.} By extending the estimator $\wh{f}$ to be defined out-of-sample, extension of Laplacian Eigenmaps by kernel smoothing gives us the option of selecting the parameters $\varepsilon$ and $K$ in a data-dependent manner---e.g. by sample splitting or cross-validation. This is by far more the most common practical recommendation for selecting hyper-parameters \textcolor{red}{(references)}, and addresses a serious limitation of Laplacian Eigenmaps. Moreover, it is known \textcolor{red}{(references)} that choosing hyper-parameters by sample splitting results in estimators that optimality adapt to the order of regularity $s$---that is, that are rate-optimal (up to $\log$ factors), even when $s$ is unknown. We believe that similar arguments will imply that $T_{h,n}\wh{f}$ is adaptive in this sense, though we do not pursue the details.
	...
	\end{equation*}
	A particularly popular version of this general approach takes $\mc{F}$ to be an RKHS, which encompasses thin-plate spline interpolation (where $\mc{F} = H^s(P)$ for $s > d/2$) as a special case. Naturally, this approach works well when $f$ is close to a function $u \in \mc{F}$ with reasonably small norm.  This holds true when $f \in H^s(P;M)$ and $s > d/2$, but as already discussed when $s \leq d/2$ the Sobolev space $H^s(P)$ is not an RKHS, and in fact when $s \leq d/2$ thin-plate spline interpolation is an ill-posed problem \citep{green93}. Another, arguably simpler approach is to extend $\wh{f}$ to be piecewise constant on the Voronoi tessellation induced by $X_1,\ldots,X_n$, or equivalently to perform $1$-nearest neighbors regression on $\wh{f}$. However, this approach is theoretically optimal only when the regression function $f_0 \in C^1(P)$, in contrast to the kernel smoothing method we propose and study.
...
\paragraph{Comparison with kernel smoothing.}
It is natural to ask whether the two stage estimator $T_{h,n}\wh{f}$ defined in Section~\ref{sec:out_of_sample} has any advantage over the simpler approach of directly kernel smoothing the responses, i.e. using the estimator $T_{h,n}Y$ (possibly for a different choice of $h$.) In Appendix~\textcolor{red}{(?)}, we answer this question in the affirmative, by giving a simple example of a sequence of densities and regression functions $\{(p^{(n)}, f_0^{(n)}: n \in \mathbb{N}\}$ such that $\|f_0 - T_{h,n}\wh{f}\|_P^2 \ll \inf_{h'} \|f_0 - T_{h',n}Y\|_P^2$. This is possible because Laplacian eigenmaps induces a completely different bias than kernel smoothing, which depends on the relationship between $f_0$ and $p$. When $f_0$ and $p$ satisfy the so-called \emph{cluster} assumption--- i.e. $f_0$ is piecewise constant in high-density regions of $p$--- then the bias of Laplacian smoothing can much smaller than that of kernel smoothing (for equivalent levels of variance). 

We emphasize that this does not contradict the well-known fact that kernel smoothing is rate-optimal for nonparametric regression. It simply reflects that in the standard nonparametric regression setup---which we adopt in the main part of this paper, and in which $P$ is assumed to be equivalent to Lebesgue measure--the biases of Laplacian eigenmaps and kernel smoothing are, up to constants, equivalent. There has been some work \textcolor{red}{(Wasserman, Niyogi, El Alaoui)} analyzing semi-supervised learning in the case where $p$ is very non-uniform, and $f_0$ ``adheres to'' $p$ in some way, such as the cluster assumption. However, a comprehensive analysis of the biases induced by Laplacian method as a function of $p$, how they differ with other approaches to estimation, and the consequences for nonparametric regression, remain outstanding. 
