...
\begin{figure*}[tb]
	\includegraphics[width=.245\textwidth]{figures/in_sample_mse/mse_by_sample_size_1d_1s.pdf}
	\includegraphics[width=.245\textwidth]{figures/in_sample_mse/mse_by_sample_size_1d_2s.pdf}
	\includegraphics[width=.245\textwidth]{figures/in_sample_mse/mse_by_sample_size_2d_1s.pdf}
	\includegraphics[width=.245\textwidth]{figures/in_sample_mse/mse_by_sample_size_2d_2s.pdf}
	\caption{Mean squared error of Laplacian eigenmaps (\texttt{LE}) and a spectral projection estimator (\texttt{SP}) as a function of sample size $n$. Each plot is on the log-log scale, and the results are averaged over 400 repetitions. Both estimators are tuned for optimal average mean squared error. The black line shows the minimax rate (in slope only; the intercept is chosen to match the observed error).}
	\label{fig:fig1}
\end{figure*}

We also compare the error, over a held-out test set, of Laplacian eigenmaps plus kernel smoothing to the spectral projection estimator. The relative performances of the two estimators, as seen in Figure~\ref{fig:fig2}, is very similar to that seen in Figure~\ref{fig:fig1}, supporting our theoretical claim that the additional error incurred by kernel smoothing of Laplacian Eigenmaps is negligible.
\begin{figure*}[tb]
	...
	\caption{Mean squared error of Laplacian eigenmaps plus kernel smoothing (\texttt{LE+KS}) and a spectral projection estimator (\texttt{SP}) as a function of sample size $n$. Each plot is on the log-log scale, and the results are averaged over 200 repetitions. Both estimators are tuned for optimal average mean squared error. The black line shows the minimax rate (in slope only; the intercept is chosen to match the observed error).}
	\label{fig:fig2}
\end{figure*}

In Figure~\ref{fig:fig3} we compare tests using Laplacian eigenmaps and spectral projection. \textcolor{red}{(TODO): Need to ask Siva for help.}
\begin{figure*}[tb]
...
\begin{figure*}[tb]
	...
	\caption{Mean squared error of Laplacian Eigenmaps as a function of hyperparameters. For all experiments, sample size $n = 2000$, and the results are averaged over $200$ repetitions. Colors show~\textcolor{red}{(?)}}
	\label{fig:fig4}
\end{figure*}
